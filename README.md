...
- **Flags**: include `-host`, `-model`, `-chunk-size`, `-chunk-overlap`, `-force`, `-dry-run`, `-max-files`, `-verbose`, `-quiet`, `-exclude <regex>` (repeatable), `-request-timeout <duration>`, `-version`.
...
1. **Init**: parse flags, validate the required `rootPath`, derive the HTTP timeout (default 2m, overridable via `-request-timeout`), negotiate the model (override > autodetect > fallback list).
